...
static int uleaf_chop(struct btree *btree, tuxkey_t start, u64 len,vleaf *vleaf)
{
	...
	/* Chop all range if len >= MAX_TUXKEY + 1 */
	stop = (len >= MAX_TUXKEY + 1) ? MAX_TUXKEY + 1 : start + len;
...
static void test02(struct sb *sb, struct inode *inode)
{
	...
	/* Delte all */
	test_assert(btree_chop(btree, 0, MAX_TUXKEY + 1) == 0);
...
static void test03(struct sb *sb, struct inode *inode)
{
	...
	for (int key = keys - 1; key >= left; key--) {
		test_assert(btree_chop(btree, key, MAX_TUXKEY + 1) == 0);
...
static void test04(struct sb *sb, struct inode *inode)
{
	...
	free_cursor(cursor);
	free_cursor(cursor2);
	test_assert(!btree_chop(btree, 0, MAX_TUXKEY + 1));
...
static void test06(struct sb *sb, struct inode *inode)
{
	 ...
	 * (6) and (11). And adjust_parent_sep() adjust (8) to (12).
	 *
	 * [(dummy) is to prevent merge nodes of (2,5) and (9,12)]
	 */
	...
	int node_key[3][4] = {
		{ 1, 8, 0, 0, }, /* child pointer is filled later */
		{ 2, 5, bufindex(leaf[0]), bufindex(leaf[1]), },
		{ 9, 12, bufindex(leaf[2]), bufindex(leaf[3]), },
	};
